...
//ciphertext 128_16 first entry original: 0xe5, sanity test: 0x52
//clone of the above because complexity
/* static const struct TestVector attackVector  = {
    .name        = "ATTACK VECTOR HARDCODE EXAMPLE",
    .key1        = {0x27, 0x18, 0x28, 0x18, 0x28, 0x45, 0x90, 0x45,
                    0x23, 0x53, 0x60, 0x28, 0x74, 0x71, 0x35, 0x26},
    .key2        = {0x31, 0x41, 0x59, 0x26, 0x53, 0x58, 0x97, 0x93,
                    0x23, 0x84, 0x62, 0x64, 0x33, 0x83, 0x27, 0x95},
    ...
    .ciphertext  = {0xe5, 0x9e, 0x6f, 0x23, 0x3b, 0xe0, 0xe0, 0x83,
                    ...
                    0xa5, 0x53, 0xa5, 0x94, 0x81, 0x45, 0xa6, 0xff,
                    0xb5, 0xe6, 0xbe, 0x1d, 0xb5},
    .tweak       = {0xff, 0xff, 0xff, 0xff, //0x80 first byte
                    0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff},
    .sectorSize  = 37
...
//.tweak       = {0x33, 0x22, 0x11, 0x00},
//2048 sectorSize plaintext 
//    .tweak       = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
//    .tweak       = {0x01, 0x00, 0xaa, 0x55, 0xd8, 0x54, 0x12, 0x76, 0x56, 0x08, 0x28, 0x7d, 0xd3, 0x81, 0x96, 0x16},
// FIRST TEST: ORIGINAL TWEAK PRE ENCRYPT: {0xe0, 0x41, 0xa9, 0xf1, 0x7e, 0xc8, 0xf0, 0x81, 0x99, 0xa2, 0xd3, 0x14, 0xee, 0x4c, 0x18, 0x4a}
// RUN TWO PRE ENCRYPT: {0x14, 0x3f, 0x9a, 0x76, 0xb2, 0xc7, 0xd9, 0x45, 0x1e, 0x88, 0x32, 0xff, 0x62, 0xa4, 0x3c, 0x7e}
...
void setAndTestTweak() {
    //first 10 bits of 128 bit tweak vector is set to 1. 
    /*uint8_t tweakBytes[16] = {0xFF, 0xC0, 0x00, 0x00, 
                              0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00};*/
    uint8_t tweakBytes[16] = {0xFF, 0xFF, 0xFF, 0xFF, 
                              0xFF, 0xFF, 0xFF, 0xFF,
                              0xFF, 0xFF, 0xFF, 0xFF,
                              0xFF, 0xFF, 0xFF, 0xFF};

    uint8_t tweakBytes2[16] = {0x00, 0x00, 0x00, 0x00, 
                              0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00};

    // Convert to uint32_t array
    uint32_t tweakUInt32one[4];
    std::memcpy(tweakUInt32one, tweakBytes, sizeof(tweakUInt32one));

    uint32_t tweakUInt32two[4];
    std::memcpy(tweakUInt32two, tweakBytes2, sizeof(tweakUInt32two));

    // Pass to updateTweak 5 times
    for (int i = 0; i < 5; i++) {
        updateTweak(tweakUInt32one);
        updateTweak(tweakUInt32two);
    }
}

uint8_t tenTweak(uint8_t cmd, uint8_t scmd, uint8_t len, uint8_t *buf) {
    setAndTestTweak();
    return 0x00;
}

uint8_t attack_example(uint8_t cmd, uint8_t scmd, uint8_t len, uint8_t *buf) {
    ...
    //trigger_high();
    cipher.setSectorSize(attackVector.sectorSize);
    cipher.setKey(attackVector.key1, 32); // Assuming 32 bytes for key1 and key2 combined
    cipher.setTweak(attackVector.tweak, sizeof(attackVector.tweak));
    cipher.encryptSector(buffer, attackVector.plaintext, iter); // In-place encryption
    //cipher.encryptSector(buffer, attackVector.plaintext); // In-place encryption

    //if (memcmp(buffer, attackVector.ciphertext, attackVector.sectorSize)) {
    //    passed = false;
    //}

    //uint8_t result = passed ? 0x00 : 0x01; // Return 0x00 for success, 0x01 for failure
    //simpleserial_put('s', 1, &result); // Now using 's' as the command both for request and response
    //trigger_low();
    //return result;
...
int main() {
    ...
    //simpleserial_addcmd('s', 0, attack_example);
    // with iter:
    simpleserial_addcmd('s', 1, attack_example);

    simpleserial_addcmd('c', 0, tenTweak);
    //simpleserial_addcmd('s', 0, run_single);
